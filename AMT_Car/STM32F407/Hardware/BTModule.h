...
#define FRAME_BYTE_LENGTH 7 //串口通讯一帧数据的字节数（含帧头和帧尾），譬如20个字节为一个完整的数据帧，第1个字节帧头，第2个字节代表命令类型，第3~6字节是命令参数，第7个字节为帧尾
#define FRAME_START 0xA5 //帧头
#define FRAME_END 0xAA  //帧尾
...
void USART_SendByte(USART_TypeDef* USARTx,u8 data);  //发送一个字节
void USART_Send2Byte(USART_TypeDef* USARTx,u16 data);  //发送两个字节
